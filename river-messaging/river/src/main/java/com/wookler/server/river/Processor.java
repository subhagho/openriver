...
public abstract class Processor<M> implements Configurable {
    ...
    /** Processor's setState instance */
    protected ProcessState state = new ProcessState();
    /** whether the Processor should ignore exceptions? */
    ...
    public void configure(ConfigNode config) throws ConfigurationException {
        try {
            ...
            log.info("Ignore exception flag for processor [ " + name + " ] is set to [ "
                    + ignoreException + " ]");
        } catch (Exception ex) {
    ...
    /**
     * Instantiates a new processor. Creates a unique id and assigns it as id
     * and name for this Processor
     */
    protected Processor() {
    ...
    /**
     * Register counters for this Processor (number of messages, number of
     * successful messages, number of failed messages, exception count,
     * processing time)
     */
    private void registerCounters() {
        AbstractCounter c = Monitoring.create(Constants.MONITOR_NAMESPACE,
                Constants.MONITOR_COUNTER_MESSAGES, Count.class, AbstractCounter.Mode.PROD);
        if (c != null) {
            counters.put(Constants.MONITOR_COUNTER_MESSAGES,
                    new String[] { c.namespace(), c.name() });
        }
        c = Monitoring.create(Constants.MONITOR_NAMESPACE, Constants.MONITOR_COUNTER_SUCCESS,
                Count.class, AbstractCounter.Mode.PROD);
        if (c != null) {
            counters.put(Constants.MONITOR_COUNTER_SUCCESS,
                    new String[] { c.namespace(), c.name() });
        }
        c = Monitoring.create(Constants.MONITOR_NAMESPACE, Constants.MONITOR_COUNTER_FAILED,
                Count.class, AbstractCounter.Mode.PROD);
        if (c != null) {
            counters.put(Constants.MONITOR_COUNTER_FAILED, new String[] { c.namespace(), c.name() });
        }
        c = Monitoring.create(Constants.MONITOR_NAMESPACE, Constants.MONITOR_COUNTER_EXCEPTION,
                Count.class, AbstractCounter.Mode.PROD);
        if (c != null) {
            counters.put(Constants.MONITOR_COUNTER_EXCEPTION,
                    new String[] { c.namespace(), c.name() });
        }
        c = Monitoring.create(Constants.MONITOR_NAMESPACE, Constants.MONITOR_COUNTER_TIME,
                Average.class, AbstractCounter.Mode.PROD);
        if (c != null) {
            counters.put(Constants.MONITOR_COUNTER_TIME, new String[] { c.namespace(), c.name() });
        }
    }

    /**
     * Increment the specified counter with the specified value
     *
     * @param name
     *            the counter name
     * @param value
    ...
    /**
     * Stop the timer corresponding to average time and update the counter
     *
     * @param starttime
     *            the starttime of this operation
     * @param count
     ...
     */
    public ProcessResponse<M> execute(List<Message<M>> messages) throws ProcessingException,
        ...
        try {
            ...
            if (resp.response() == EProcessResponse.Success) {
                incrementCounter(Constants.MONITOR_COUNTER_SUCCESS, messages.size());
            } else if (resp.response() == EProcessResponse.Failed) {
                incrementCounter(Constants.MONITOR_COUNTER_FAILED, messages.size());
            } else if (resp.response() == EProcessResponse.Exception) {
                incrementCounter(Constants.MONITOR_COUNTER_EXCEPTION, messages.size());
            }
            return resp;
    ...
    /**
     * Execute this processor on the message passed.
     * 
     * @param message
     *            - Message.
     * @return - Execution response.
     * @throws ProcessingException
     */
    public ProcessResponse<M> execute(Message<M> message) throws ProcessingException,
            NonFatalProcessorException {
        if (message == null)
            return new ProcessResponse<M>().response(EProcessResponse.Failed);
        long ts = Monitoring.timerstart();
    ...
    /**
     * Method to be implemented for processing list of message blocks.
     * 
     * @param messages
     ...
     * @throws ProcessingException
     */
    protected abstract ProcessResponse<M> process(Message<M> message) throws ProcessingException,
            NonFatalProcessorException;
}
