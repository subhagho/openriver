...
/**
 * Server context corresponding to the Queue REST Server. This is responsible
 * for reading the server config params. It is also responsible for initializing
 * and configuring the specified queue(s) (if any). Maintains a list of
 * configured queues and provides handle to start the queues.
 * 
...
public class ServerContext implements Configurable {
    ...
    public void configure(ConfigNode config) throws ConfigurationException {
        try {
            if (config == null)
                throw new ConfigurationException("Null configuration node passed.");
            ...
            if (sn == null)
                throw new ConfigurationException("Server configuration not defined. [node="
            ...
            if (sa.contains(Constants.CONFIG_NUM_THREADS)) {
                n_threads = Integer.parseInt(sa.param(Constants.CONFIG_NUM_THREADS));
            }
            ...
            if (sslmode) {
                if (sa.contains(Constants.CONFIG_SSL_PORT)) {
                    q_sslPort = Integer.parseInt(sa.param(Constants.CONFIG_SSL_PORT));
                }
            ...
            if (sa.contains(Constants.CONFIG_COMPRESS)) {
                compress = Boolean.parseBoolean(sa.param(Constants.CONFIG_COMPRESS));
            }
            ...
            throw new ConfigurationException("Error initializing Server Context.", e);
        }
    }

    /**
     * Initialize and configure the message queue(s)
     * 
     * @param config
     *            configuration node/list corresponding to the queue(s)
     * @throws ConfigurationException
     */
        ...
        if (config instanceof ConfigPath) {
            ...
            for (ConfigNode n : cv.values()) {
                if (!(n instanceof ConfigPath))
                    throw new ConfigurationException(String.format(
                            "Invalid config node type. [expected:%s][actual:%s]", ConfigPath.class
                                    .getCanonicalName(), config.getClass().getCanonicalName()));
    ...
    /**
     * Initialize the Message Queue wand configure it with specified params. Add
     * to message queue map
     * 
     * @param config
     *            config node corresponding to the queue
     * @throws ConfigurationException
     */
    private void configQueue(ConfigPath config) throws ConfigurationException {
        ...
        try {
            ...
            if (!(o instanceof MessageQueue)) {
                throw new ConfigurationException("Invalid Queue implementation specified. [class="
    ...
    public void startQueues() throws ServerException {
        try {
            ...
            throw new ServerException(QueueRestServer.class, "Invalid Context setState.", e);
    ...
    public MessageQueue<?> queue(String name) throws ServerException {
        try {
            ...
            throw new ServerException(QueueRestServer.class, "Invalid setState", e);
        }
    }
