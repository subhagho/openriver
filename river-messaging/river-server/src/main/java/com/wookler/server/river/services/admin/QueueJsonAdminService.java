...
/**
 * JSON based service to provide Queue Server Admin functions.
 *
...
public class QueueJsonAdminService extends AbstractQueueService {
	private static final Logger log = LoggerFactory
			.getLogger(QueueJsonAdminService.class);
	...
	public JResponse<RestResponse> open(@Context final HttpServletRequest req)
		...
		try {
			...
			response.setStatus(EServiceResponse.Exception).getStatus()
					.setError(t);
		}
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public JResponse<RestResponse> status(
		...
		try {
			...
			if (!StringUtils.isEmpty(subscriber)
					&& subscriber.compareToIgnoreCase("all") != 0) {
				Subscriber<?> s = q.subscriber(subscriber);
				if (s == null)
					throw new ServiceException("No subscriber found. [name="
							+ subscriber + "]");
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public JResponse<RestResponse> shutdown(
			@Context final HttpServletRequest req, String password)
			throws ServiceException {
